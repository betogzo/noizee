...
extension SingletonPlayerWebView {
    ...
    func injectBlackoutCSS() {
        ...
        let script = """
            (function() {
                ...
                if (!document.getElementById('noizee-blackout')) {
                    const blackout = document.createElement('div');
                    blackout.id = 'noizee-blackout';
            ...
            (function() {
                ...
                if (playerPage && typeof playerPage.videoMode !== 'undefined') {
                    if (playerPage.videoMode !== true) {
                        ...
                        console.log('[Noizee] Forced videoMode = true via property');
                ...
                if (switcher) {
                    ...
                    if (videoBtn && !videoBtn.hasAttribute('active')) {
                        videoBtn.click();
                        console.log('[Noizee] Clicked Video toggle via switcher');
                ...
                for (const btn of allButtons) {
                    ...
                    if (text === 'video') {
                        ...
                        if (!isActive) {
                            btn.click();
                            console.log('[Noizee] Clicked Video button by text');
    ...
    private func removeBlackoutOnly() {
        self.webView?.evaluateJavaScript("const b = document.getElementById('noizee-blackout'); if (b) b.remove();")
    }
    ...
    private static func videoContainerScriptInPlace() -> String {
        ...
        (function() {
            ...
            const styleId = 'noizee-video-mode-style-v2';
            ...
            style.textContent = `
                ...
                video.noizee-visible, .video-stream.noizee-visible {
            ...
            const markAncestors = () => {
                ...
                // Clear old marks
                document.querySelectorAll('.noizee-visible').forEach(el => el.classList.remove('noizee-visible'));

                // Mark current video and all its parents
                let current = video;
                while (current && current !== document.documentElement) {
                    current.classList.add('noizee-visible');
            ...
            const enforceVisibility = () => {
                markAncestors();
                const video = document.querySelector('video');
                if (video && typeof window.__noizeeVideoModeActive === 'boolean' && window.__noizeeVideoModeActive) {
                ...
                if (window.__noizeeVideoModeActive) {
                    requestAnimationFrame(enforceVisibility);
                }
            };
            window.__noizeeVideoModeActive = true;
            requestAnimationFrame(enforceVisibility);

            // Remove the extraction container if it exists from previous attempts
            const oldContainer = document.getElementById('noizee-video-container');
            if (oldContainer) {
                const video = oldContainer.querySelector('video');
                if (video && video.__noizeeOriginalParent) {
                    video.__noizeeOriginalParent.appendChild(video);
                    delete video.__noizeeOriginalParent;
                }
                oldContainer.remove();
            }
    ...
    func removeVideoModeCSS() {
        ...
        let script = """
            (function() {
                ...
                const blackout = document.getElementById('noizee-blackout');
                if (blackout) blackout.remove();

                // Remove the in-place style
                const style = document.getElementById('noizee-video-mode-style-v2');
                if (style) style.remove();

                // Stop the enforcement loop
                window.__noizeeVideoModeActive = false;
