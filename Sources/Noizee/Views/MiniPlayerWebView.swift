...
final class SingletonPlayerWebView {
    ...
    func loadVideo(videoId: String, strategy: VideoLoadStrategy = .standard) {
        ...
        webView.evaluateJavaScript("document.querySelector('video')?.pause()") { [weak self] _, _ in
            ...
            let prepareScript = "window.__noizeeTargetVolume = \(currentVolume);"
    ...
    nonisolated static func autoplayIntentScript(isRestoringPlaybackSession: Bool) -> String {
        "window.__noizeeAutoplayPending = \(isRestoringPlaybackSession ? "false" : "true");"
    }
    ...
    nonisolated static func pageBootstrapScript(
        ...
        return """
            ...
            window.__noizeeTargetVolume = \(clampedVolume);
        """
    }
            ...
            let applyVolumeScript = """
                (function() {
                    try {
                        ...
                        window.__noizeeTargetVolume = volume;
                        window.__noizeeIsSettingVolume = true;
                        ...
                        setTimeout(() => { window.__noizeeIsSettingVolume = false; }, 100);
...
extension SingletonPlayerWebView {
    ...
    /// Re-applies ``mediaControlOverrideScript`` wiring when the window or app is occluded.
    /// The override loop relies on `requestAnimationFrame`, which stalls when Noizee is hidden,
    /// allowing YouTube to reclaim Media Session handlers so hardware keys no longer reach ``PlayerService``.
    func refreshMediaSessionControlOverrides() {
        guard let webView = self.webView else { return }
        let script = """
            try {
                if (typeof window.__noizeeRefreshMediaControlStyle === 'function') {
                    window.__noizeeRefreshMediaControlStyle();
                }
            } catch (e) {}
        """
        webView.evaluateJavaScript(script, completionHandler: nil)
    ...
    static func mediaControlStyleBootstrapScript(useNextPrev: Bool) -> String {
        ...
        return """
            (function() {
                try {
                    localStorage.setItem('noizeeUseNextPrev', '\(jsBoolean)');
                } catch (e) {}
                window.__noizeeUseNextPrev = \(jsBoolean);
                ...
                try {
                    ...
                    if (ms && !ms.__noizeeSetActionHandlerWrapped) {
                        var orig = ms.setActionHandler.bind(ms);
                        ms.setActionHandler = function(type, handler) {
                            if (window.__noizeeUseNextPrev && (type === 'seekforward' || type === 'seekbackward')) {
                                return orig(type, null);
                            }
                            return orig(type, handler);
                        };
                        ms.__noizeeSetActionHandlerWrapped = true;
                    }
                } catch (e) {}
            })();
        """
    ...
    static var mediaControlOverrideScript: String {
        """
        (function() {
            if (typeof window.__noizeeUseNextPrev !== 'boolean') {
                try {
                    window.__noizeeUseNextPrev =
                        localStorage.getItem('noizeeUseNextPrev') === 'true';
                } catch (e) {
                    window.__noizeeUseNextPrev = false;
                }
            }

            var overrideFrameId = null;
            var hiddenOverridePollId = null;

            function applyOverride() {
                if (!window.__noizeeUseNextPrev) {
                    return;
                }
                try {
                    var ms = navigator.mediaSession;
            ...
            function stopHiddenOverridePolling() {
                if (hiddenOverridePollId !== null && typeof clearInterval === 'function') {
                    clearInterval(hiddenOverridePollId);
                    hiddenOverridePollId = null;
                }
            }

            function startHiddenOverridePollingIfNeeded() {
                stopHiddenOverridePolling();
                if (typeof setInterval !== 'function') {
                    return;
                }
                if (!document.hidden || !window.__noizeeUseNextPrev) {
                    return;
                }
                // When the window is hidden/minimized, rAF is throttled or paused, so YouTube can
                // reclaim Media Session handlers and hardware keys stop routing through Noizee.
                hiddenOverridePollId = setInterval(function() {
                    applyOverride();
                }, 400);
            }

            function scheduleOverrideLoop() {
                if (overrideFrameId !== null || !window.__noizeeUseNextPrev) {
                    return;
                }
            ...
            window.__noizeeRefreshMediaControlStyle = function() {
                overrideFrameId = null;
                applyOverride();
                startHiddenOverridePollingIfNeeded();
                scheduleOverrideLoop();
            };
            ...
            function attachVideoOverride() {
                var v = document.querySelector('video');
                if (!v || v.__noizeeOverrideAttached) return;
                v.__noizeeOverrideAttached = true;
