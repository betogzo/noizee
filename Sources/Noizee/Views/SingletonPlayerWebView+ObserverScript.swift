...
extension SingletonPlayerWebView {
    ...
    nonisolated static var autoplayRecoveryFunctionJS: String {
        """
        function __noizeeAttemptAutoplayRecovery(video, playBtn) {
            if (!window.__noizeeAutoplayPending) return 'noop';
            if (!video.paused) { window.__noizeeAutoplayPending = false; return 'noop'; }
            window.__noizeeAutoplayPending = false;
    ...
    nonisolated static var observerScript: String {
        ...
        (function() {
            ...
            let lastPrimedArtistByVideoId = Object.create(null);

            // Volume enforcement: track target volume set by Swift
            // Don't set a default - only enforce when explicitly set by Swift
            // window.__noizeeTargetVolume is set by volume init script at document start
            ...
            function enforceVolumeNow() {
                const targetVol = window.__noizeeTargetVolume;
            ...
            function setupVideoListeners() {
                ...
                function attachVideoListeners() {
                    ...
                    if (video.__noizeeListenersAttached) return;
                    video.__noizeeListenersAttached = true;
                    ...
                    video.addEventListener('webkitcurrentplaybacktargetiswirelesschanged', () => {
                        ...
                        const wasConnected = window.__noizeeAirPlayConnected;
                        window.__noizeeAirPlayConnected = isWireless;

                        bridge.postMessage({
                            type: 'AIRPLAY_STATUS',
                            isConnected: isWireless,
                            wasConnected: wasConnected,
                            wasRequested: window.__noizeeAirPlayRequested || false
                        });
                    });
                    ...
                    if (initialWireless) {
                        window.__noizeeAirPlayConnected = true;
                        bridge.postMessage({
                            type: 'AIRPLAY_STATUS',
                            isConnected: true,
                            wasConnected: false,
                    ...
                    } else if (window.__noizeeAirPlayRequested && window.__noizeeAirPlayConnected) {
                        window.__noizeeAirPlayConnected = false;
                        bridge.postMessage({
                            type: 'AIRPLAY_STATUS',
                            isConnected: false,
                            wasConnected: true,
                    ...
                    video.addEventListener('volumechange', () => {
                        ...
                        if (window.__noizeeIsSettingVolume) return;
                        enforceVolumeNow();
                    });
                ...
                const videoObserver = new MutationObserver(() => {
                    const video = document.querySelector('video');
                    if (video && !video.__noizeeListenersAttached) {
                        attachVideoListeners();
                    }
                });
            ...
            function sendUpdate() {
                ...
                try {
                    ...
                    // Byline can clear for a frame while the title is already updated for the same video.
                    if (!artist && videoId && videoId === lastVideoId && lastPrimedArtistByVideoId[videoId]) {
                        artist = lastPrimedArtistByVideoId[videoId];
                    }
                    ...
                    if (trackChanged && videoIdChanged && videoId !== '') {
                        for (const k of Object.keys(lastPrimedArtistByVideoId)) {
                            if (k !== videoId) delete lastPrimedArtistByVideoId[k];
                        }
                    }
                    if (artist && videoId) {
                        lastPrimedArtistByVideoId[videoId] = artist;
                    }
