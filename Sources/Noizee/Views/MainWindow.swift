...
struct MainWindow: View {
    ...
    /// Marks the bundled app version's What's New as already presented once per installation so the
    /// Highlights sheet cannot appear automatically during first-run setup (`WhatsNewHelp` keeps working).
    private enum BundledWhatsNewSeeding {
        private static let userDefaultsKey = "Noizee.didSeedBundledWhatsNewVersionAsPresented"

        static func runIfNeeded() {
            guard !UserDefaults.standard.bool(forKey: self.userDefaultsKey) else { return }
            WhatsNewVersionStore().markPresented(WhatsNew.Version.current())
    ...
    var body: some View {
        ...
        .onAppear {
            BundledWhatsNewSeeding.runIfNeeded()
        }
        ...
        .toolbar {
            ToolbarItemGroup(placement: .primaryAction) {
                HStack(spacing: 10) {
                    ...
                    if self.navigationSelection != .search, let searchVM = self.searchViewModel {
                        ToolbarSearchField(
                            viewModel: searchVM,
    ...
    private func handleAuthStateChange(oldState: AuthService.State, newState: AuthService.State) {
        ...
        case .loggedIn:
            ...
            // Do not auto-present What's New on login; use Help → "What's New in Noizee" instead.
            Task {
...
/// Compact search field in the window toolbar (all main routes except the dedicated Search screen).
@available(macOS 26.0, *)
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            TextField(String(localized: "Search songs, albums, artists..."), text: self.$viewModel.query)
                ...
                .accessibilityIdentifier(AccessibilityID.MainWindow.toolbarSearchField)
