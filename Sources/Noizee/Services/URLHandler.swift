...
/// - `noizee://play?v=dQw4w9WgXcQ` - Custom scheme for song
/// - `noizee://playlist?list=PLxxx` - Custom scheme for playlist
/// - `noizee://album?id=MPRExxx` - Custom scheme for album
/// - `noizee://artist?id=UCxxx` - Custom scheme for artist
enum URLHandler {
    // MARK: - Types
    ...
    static func parse(_ url: URL) -> ParsedContent? {
        ...
        if url.scheme == "noizee" {
            return self.parseNoizeeURL(url)
        }
    ...
    /// Parses a noizee:// custom scheme URL.
    private static func parseNoizeeURL(_ url: URL) -> ParsedContent? {
        guard url.scheme == "noizee" else { return nil }
