...
enum ParsingHelpers {
    ...
    /// Type labels YouTube Music places before creator names in subtitles (not performer names).
    private static let musicContentTypeLabels: Set<String> = [
        "Song", "Video", "Album", "Playlist", "Episode", "Podcast",
        "EP", "E.P.", "Single", "Singles", "Remix",
    ]

    /// Whether `text` is a music item type label (e.g. **Song**, **EP**), possibly with trailing punctuation.
    static func isMusicContentTypeLabel(_ text: String) -> Bool {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return false }
        let withoutPunctuation = trimmed.trimmingCharacters(in: CharacterSet(charactersIn: ",·•"))
        let base = withoutPunctuation.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !base.isEmpty else { return false }
        return Self.musicContentTypeLabels.contains { $0.caseInsensitiveCompare(base) == .orderedSame }
    }
           ...
           let runs = subtitleData["runs"] as? [[String: Any]]
        {
            for run in runs {
                if let text = run["text"] as? String,
                   text != " • ", text != " & ", text != ", ",
                   !Self.isMusicContentTypeLabel(text)
                {
    ...
    /// Best-effort section title for browse/search shelf renderers (`musicShelfRenderer`, carousels, etc.).
    static func browseShelfTitle(forRenderer data: [String: Any]) -> String? {
        if let header = data["header"] as? [String: Any] {
            for key in [
                ...
                if let renderer = header[key] as? [String: Any],
                   let title = self.extractTitle(from: renderer)
                {
                    return title
                }
            }
    ...
    /// Whether a shelf title denotes YouTube Music's **Videos** category (as opposed to Songs, albums, etc.).
    ///
    /// Section titles are localized; this uses diacritic-insensitive matching on a small set of common headings.
    static func isVideosCategoryShelfTitle(_ title: String) -> Bool {
        ...
        switch normalized {
        case "videos", "video", "music videos", "music video":
            return true
        default:
            return false
        }
           ...
           let runs = text["runs"] as? [[String: Any]]
        {
            for run in runs {
                if let artistName = run["text"] as? String,
                   ...
                   !artistName.isEmpty,
                   !Self.isMusicContentTypeLabel(artistName)
                {
    ...
    static func extractAlbumFromFlexColumns(_ data: [String: Any]) -> Album? {
        ...
        for columnIndex in 1 ..< min(4, flexColumns.count) {
            guard let column = flexColumns[safe: columnIndex],
                  ...
                  let text = renderer["text"] as? [String: Any],
                  let runs = text["runs"] as? [[String: Any]],
                  let album = Self.extractAlbumFromNavigationRuns(runs)
            else {
                continue
            }
            return album
    ...
    /// Extracts album from `subtitle` runs (`musicTwoRowItemRenderer` shelves often carry album links here).
    static func extractAlbumFromSubtitleRuns(_ data: [String: Any]) -> Album? {
        guard let subtitleData = data["subtitle"] as? [String: Any],
              let runs = subtitleData["runs"] as? [[String: Any]]
        else {
            return nil
        }
        return Self.extractAlbumFromNavigationRuns(runs)
    }

    /// First run in `runs` whose navigation endpoint is an album/single browse ID (MPRE / OLAK).
    private static func extractAlbumFromNavigationRuns(_ runs: [[String: Any]]) -> Album? {
        for run in runs {
    ...
    // MARK: - YouTube Music Web Player DOM

    /// YouTube Music's hidden web player bar byline often concatenates **artist**, **album**, and **year**
    /// with middle dots (e.g. `Artist • Album • 2020`). Noizee should show only the artist segment
    /// in notifications and the UI subtitle line.
    static func primaryArtistLine(fromYouTubeMusicPlayerBarByline byline: String) -> String {
        ...
        if let bullet = trimmed.firstIndex(of: "•") {
            let head = trimmed[..<bullet].trimmingCharacters(in: .whitespacesAndNewlines)
            return head.isEmpty ? trimmed : head
        }
