...
enum HomeResponseParser {
    ...
    static func parse(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> HomeResponse {
        ...
        for sectionData in sectionContents {
            if let section = parseHomeSection(sectionData, excludeVideosCategoryShelves: excludeVideosCategoryShelves) {
    ...
    static func parseContinuation(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> [HomeSection] {
        var sections: [HomeSection] = []
        ...
        if let continuationContents = data["continuationContents"] as? [String: Any] {
            ...
            if let shelfContinuation = continuationContents["musicShelfContinuation"] as? [String: Any] {
                if let section = parseMusicShelf(shelfContinuation, excludeVideosCategoryShelves: excludeVideosCategoryShelves) {
                    sections.append(section)
                }
            }
        }
    ...
    static func parseHomeSection(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> HomeSection? {
        ...
        if let carouselRenderer = data["musicCarouselShelfRenderer"] as? [String: Any] {
            return self.parseMusicCarouselShelf(carouselRenderer, excludeVideosCategoryShelves: excludeVideosCategoryShelves)
        }

        // Try musicShelfRenderer (vertical song lists)
        if let shelfRenderer = data["musicShelfRenderer"] as? [String: Any] {
            return self.parseMusicShelf(shelfRenderer, excludeVideosCategoryShelves: excludeVideosCategoryShelves)
        }
        ...
        if let cardShelfRenderer = data["musicCardShelfRenderer"] as? [String: Any] {
            return self.parseMusicCardShelf(cardShelfRenderer, excludeVideosCategoryShelves: excludeVideosCategoryShelves)
        }
        ...
        if let immersiveCarouselRenderer = data["musicImmersiveCarouselShelfRenderer"] as? [String: Any] {
            return self.parseMusicImmersiveCarouselShelf(
                immersiveCarouselRenderer,
                excludeVideosCategoryShelves: excludeVideosCategoryShelves
            )
        }

        // Try gridRenderer (used for charts and grids)
        if let gridRenderer = data["gridRenderer"] as? [String: Any] {
            return self.parseGridRenderer(gridRenderer, excludeVideosCategoryShelves: excludeVideosCategoryShelves)
        }
           ...
           let itemContents = itemSectionRenderer["contents"] as? [[String: Any]]
        {
            for itemContent in itemContents {
                if let section = parseHomeSection(itemContent, excludeVideosCategoryShelves: excludeVideosCategoryShelves) {
                    return section
                }
            }
        }
    ...
    private static func parseMusicCarouselShelf(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> HomeSection? {
        let title = ParsingHelpers.browseShelfTitle(forRenderer: data)
            ?? self.extractCarouselTitle(from: data)
            ?? "Unknown Section"

        if excludeVideosCategoryShelves, ParsingHelpers.isVideosCategoryShelfTitle(title) {
            return nil
        }
    ...
    private static func parseMusicShelf(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> HomeSection? {
        let title = ParsingHelpers.browseShelfTitle(forRenderer: data)
            ?? ParsingHelpers.extractTitle(from: data)
            ?? "Unknown Section"
    ...
    private static func parseMusicCardShelf(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> HomeSection? {
                               ...
                               let text = ParsingHelpers.extractTitle(from: headerRenderer)
        {
            ...
            ParsingHelpers.browseShelfTitle(forRenderer: data) ?? "Featured"
        }
    ...
    private static func parseMusicImmersiveCarouselShelf(
        _ data: [String: Any],
        excludeVideosCategoryShelves: Bool = false
    ) -> HomeSection? {
        let title = ParsingHelpers.browseShelfTitle(forRenderer: data)
            ?? self.extractCarouselTitle(from: data)
            ?? "Featured"
    ...
    private static func parseGridRenderer(_ data: [String: Any], excludeVideosCategoryShelves: Bool = false) -> HomeSection? {
        let title: String = if let header = data["header"] as? [String: Any],
                               ...
                               let text = ParsingHelpers.extractTitle(from: headerRenderer)
        {
            ...
            ParsingHelpers.browseShelfTitle(forRenderer: data) ?? "Charts"
        }
