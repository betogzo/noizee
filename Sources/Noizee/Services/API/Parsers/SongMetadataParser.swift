...
enum SongMetadataParser {
    ...
    static func parse(_ data: [String: Any], videoId: String) throws -> Song {
        ...
        let album = self.parseAlbum(from: panelVideoRenderer)
    ...
    static func parseArtists(from renderer: [String: Any]) -> [Artist] {
        ...
        for run in runs {
            ...
            if Self.isLikelyReleaseYearRun(text) {
                continue
            }

            let browseId: String? = if let navEndpoint = run["navigationEndpoint"] as? [String: Any],
            ...
            if let browseId, browseId.hasPrefix("MPRE") || browseId.hasPrefix("OLAK") {
                continue
            }

            let artistId: String = browseId ?? ParsingHelpers.stableId(title: "artist", components: text)
            artists.append(Artist(id: artistId, name: text))
        }
        return artists
    }

    /// Parses album from the panel video renderer's longBylineText (album runs use MPRE/OLAK browse ids).
    static func parseAlbum(from renderer: [String: Any]) -> Album? {
        guard let bylineData = renderer["longBylineText"] as? [String: Any],
              let runs = bylineData["runs"] as? [[String: Any]]
        else { return nil }
    ...
    private static func isLikelyReleaseYearRun(_ text: String) -> Bool {
        text.range(of: #"^\d{4}$"#, options: .regularExpression) != nil
    }
