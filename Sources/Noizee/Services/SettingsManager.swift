...
final class SettingsManager {
    ...
    private enum Keys {
        ...
        static let hideVideosCategoryShelves = "settings.hideVideosCategoryShelves"
    }
    ...
    /// When enabled, omit YouTube Music **Videos** shelves from Search (All) and browse-style pages so results skew toward catalog songs.
    /// Does not affect the dedicated Songs search filter or artist/album playlists.
    var hideVideosCategoryShelves: Bool {
        didSet {
            UserDefaults.standard.set(self.hideVideosCategoryShelves, forKey: Keys.hideVideosCategoryShelves)
            if oldValue != self.hideVideosCategoryShelves {
                APICache.shared.invalidateAll()
            }
        }
    }
    ...
    private init() {
        ...
        self.hideVideosCategoryShelves = UserDefaults.standard.object(forKey: Keys.hideVideosCategoryShelves) as? Bool ?? false
