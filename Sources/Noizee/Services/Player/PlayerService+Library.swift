...
extension PlayerService {
    ...
    func fetchSongMetadata(videoId: String) async {
        ...
        do {
            ...
            if self.currentTrack?.videoId == videoId {
                ...
                let currentArtists = self.currentTrack?.artists ?? []
                let webOnlyUnknownPlaceholder = currentArtists.allSatisfy { $0.id == "unknown" }
                let preferAPIArtists = currentArtists.isEmpty
                    || webOnlyUnknownPlaceholder
                    || currentArtists.allSatisfy { !$0.hasNavigableId }
                let artists: [Artist] = if preferAPIArtists, !songData.artists.isEmpty {
                    songData.artists
                } else if !currentArtists.isEmpty {
                    currentArtists
                } else {
                    songData.artists
                }

                let mergedAlbum = Self.mergedNavigableAlbumForMetadata(api: songData.album, existing: self.currentTrack?.album)
                ...
                if let queueIndex = self.queueEntries.firstIndex(where: { $0.song.videoId == videoId }) {
                    ...
                    let needsNavigableAlbum =
                        currentQueueSong.album?.hasNavigableId != true && songData.album?.hasNavigableId == true
                    ...
                    if needsUpdate {
                        let mergedForQueue = Self.mergedNavigableAlbumForMetadata(
                            api: songData.album,
                            existing: currentQueueSong.album
                        )
                        let enrichedQueueSong = Song(
                            id: songData.id,
                            title: songData.title,
                            artists: songData.artists,
                            album: mergedForQueue,
                            duration: songData.duration,
                            thumbnailURL: songData.thumbnailURL,
                            videoId: songData.videoId,
                            hasVideo: songData.hasVideo,
                            musicVideoType: songData.musicVideoType,
                            likeStatus: resolvedLikeStatus,
                            isInLibrary: songData.isInLibrary,
                            feedbackTokens: songData.feedbackTokens,
    ...
    /// Prefers a navigable album from API `getSong`, then any navigable album already known for this track.
    private static func mergedNavigableAlbumForMetadata(api: Album?, existing: Album?) -> Album? {
        if let api, api.hasNavigableId {
            return api
        }
        if let existing, existing.hasNavigableId {
            return existing
        }
        return api ?? existing
    }
