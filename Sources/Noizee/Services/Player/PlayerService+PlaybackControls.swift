...
extension PlayerService {
    ...
    func play(
        ...
        self.isNoizeeInitiatedPlayback = true
    ...
    func next() async {
        ...
        if !self.queue.isEmpty {
            if self.shuffleEnabled {
                // Pure `0..<count` random can equal `currentIndex`, which replays the same row
                // (same `videoId` -> `loadVideo(.standard)` no-ops) and feels like "next restarted this song".
                let randomIndex = Self.randomShuffleIndex(queueCount: self.queue.count, excluding: self.currentIndex)
    ...
    /// Picks a new queue index for shuffle skip, avoiding the current row when any other row exists.
    private static func randomShuffleIndex(queueCount: Int, excluding currentIndex: Int) -> Int {
        guard queueCount > 0 else { return 0 }
        guard queueCount > 1 else { return 0 }

        var pool = Array(0 ..< queueCount)
        pool.removeAll { $0 == currentIndex }
        if let chosen = pool.randomElement() {
            return chosen
        }
        return min(max(currentIndex, 0), queueCount - 1)
    }
}
