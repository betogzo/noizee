...
extension PlayerService {
    ...
    private func isRepeatAllWraparoundTrackEnd(
        ...
        // observed id before the natural `ended` callback reaches Noizee.
    ...
    private func keepQueueSongVisible(_ song: Song, thumbnailUrl: String) {
        ...
        let mergedAlbum = self.mergedAlbumWhenPreservingQueueRow(queueSong: song)
    ...
    /// When re-showing a queue row we must not drop an API/Web-enriched **navigable** album that the
    /// queue entry never had (e.g. Home carousel seeds + async `getSong` metadata).
    private func mergedAlbumWhenPreservingQueueRow(queueSong: Song) -> Album? {
        guard self.currentTrack?.videoId == queueSong.videoId else {
            return queueSong.album
        }
        let fromCurrent = self.currentTrack?.album
        let fromQueue = queueSong.album
        if let album = fromCurrent, album.hasNavigableId {
            return album
        }
        if let album = fromQueue, album.hasNavigableId {
            return album
        }
        return fromCurrent ?? fromQueue
    }
    ...
    private func handleNoizeeInitiatedPlaybackMetadata(
        observedVideoId: String?,
        title: String,
        artist: String,
        thumbnailUrl: String,
        ...
        guard self.isNoizeeInitiatedPlayback, !self.queue.isEmpty else {
            return false
        }

        guard let intendedSong = self.queue[safe: self.currentIndex] else {
            self.isNoizeeInitiatedPlayback = false
            return false
        }
        ...
        if self.observedTrackMatchesSong(
            ...
            self.isNoizeeInitiatedPlayback = false
            self.logger.debug("Confirmed Noizee-initiated playback for '\(intendedSong.title)'")
            return false
        }
    ...
    private func replayCurrentQueueSongForRepeatOneAfterTrackEnd() async {
        ...
        self.songNearingEnd = false
        let noizeeAlignedWithQueue = self.pendingPlayVideoId == currentSong.videoId
            && SingletonPlayerWebView.shared.currentVideoId == currentSong.videoId
        if self.hasUserInteractedThisSession, noizeeAlignedWithQueue {
    ...
    /// Updates track metadata and enforces Noizee's queue when YouTube tries to diverge.
    func updateTrackMetadata(title: String, artist: String, thumbnailUrl: String, videoId observedVideoId: String?) {
        let artistLine = ParsingHelpers.primaryArtistLine(fromYouTubeMusicPlayerBarByline: artist)
        self.logger.debug("Track metadata updated: \(title) — byline: \(artist, privacy: .public) → artist: \(artistLine, privacy: .public)")
        let thumbnailURL = URL(string: thumbnailUrl)
        let resolvedVideoId = self.resolvedObservedVideoId(observedVideoId)
        let trackChanged = self.currentTrack?.title != title
            || self.currentTrack?.artistsDisplay != artistLine
        ...
        if self.handleNoizeeInitiatedPlaybackMetadata(
            observedVideoId: observedVideoId,
            title: title,
            artist: artistLine,
            thumbnailUrl: thumbnailUrl,
        ...
        // Only fetch on YouTube-driven track changes; Noizee-initiated playback
    ...
    private func mergedTrackForWebMetadata(
        ...
        let webPlaceholderArtist = Artist(id: "unknown", name: artistLine)
        // YouTube often updates the title node a few ms before the byline. When `artistLine` is still
        // empty, keep any artists we already know from the queue / prior merge so the player bar
        // does not flash "Unknown Artist".
        let mergedArtists: [Artist] = if let source = metadataSource,
                                         source.artists.contains(where: \.hasNavigableId)
        {
            source.artists
        } else if !artistLine.isEmpty {
            [webPlaceholderArtist]
        } else if let source = metadataSource, !source.artists.isEmpty {
            source.artists
        } else {
            [webPlaceholderArtist]
        }
