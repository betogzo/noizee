...
/// equalizer.
///
/// **Critical detail**: Noizee doesn't decode music itself — `WKWebView` does,
...
final class ProcessTapHelper {
    ...
    private static let tapName = "com.betogzo.Noizee.EQ.Tap"

    /// User-visible name of the aggregate device that wraps the tap.
    private static let aggregateName = "Noizee EQ Aggregate"
    ...
    private static let aggregateUIDPrefix = "com.betogzo.Noizee.EQ.Aggregate."
    ...
    func start() -> Result<Void, StartFailure> {
        ...
        // they show up in Audio MIDI Setup as lingering "Noizee EQ Aggregate"
    ...
    ///
    /// Noizee's audio is decoded by `WKWebView`'s WebKit subprocesses, not
    ...
    private static func audioObjectsToTap() -> [AudioObjectID] {
        ...
        if !candidates.isEmpty {
            Self.logger.warning(
                "found \(candidates.count) WebKit audio process(es) but none were provably owned by Noizee; skipping tap to avoid hijacking unrelated audio"
            )
        }
        return []
    }
    ...
    ///
    /// WebKit helper names include the host app ("Noizee Web Content",
    /// "Noizee Graphics and Media"), which survives even when their Unix
    ...
    /// "Noizee Networking", which provides a second ownership proof path.
