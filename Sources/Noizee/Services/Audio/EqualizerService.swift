...
/// (typically wired to `PlayerService.isPlaying` from `NoizeeApp`), the
...
final class EqualizerService {
    ...
    /// coupling minimal — `NoizeeApp` wires `PlayerService.isPlaying` in.
    ...
    /// engine isn't running yet — typically called by `NoizeeApp` when
    ...
    var status: Status {
        ...
        if self.inferredPermissionDenial {
            return .permissionNeeded(message: String(
                localized: "Open System Settings → Privacy & Security → Screen & System Audio Recording and enable Noizee, then retry playback or toggle the equalizer off and on."
            ))
        }
