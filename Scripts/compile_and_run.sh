#!/usr/bin/env bash
# Reset Noizee: kill running instances, build, package, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ROOT_DIR}/.build/app/Noizee.app"
APP_PROCESS_PATTERN="Noizee.app/Contents/MacOS/Noizee"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/.build/debug/Noizee"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/.build/release/Noizee"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/noizee-compile-and-run-${LOCK_KEY}"
...
kill_all_noizee() {
  ...
  for _ in {1..25}; do
    ...
    pkill -x "Noizee" 2>/dev/null || true
    if ! is_running; then
      return 0
    fi
    sleep 0.2
  ...
  pkill -9 -x "Noizee" 2>/dev/null || true
  ...
  fail "Failed to kill all Noizee instances."
}
...
# 2) Kill all running Noizee instances.
log "==> Killing existing Noizee instances"
kill_all_noizee
...
if ! open "${APP_BUNDLE}"; then
  ...
  "${APP_BUNDLE}/Contents/MacOS/Noizee" >/dev/null 2>&1 &
  disown
fi
