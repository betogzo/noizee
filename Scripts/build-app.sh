#!/usr/bin/env bash
# Build script to create Noizee.app bundle
...
# Configuration
CONF=${1:-release}
SIGNING_MODE=${NOIZEE_SIGNING:-dev}
APP_NAME="Noizee"
BUNDLE_ID="com.betogzo.Noizee"
...
SDEF_PATH="$ROOT/Sources/Noizee/Resources/Noizee.sdef"
if [[ -f "$SDEF_PATH" ]]; then
  echo "📜 Copying AppleScript definition..."
  cp "$SDEF_PATH" "$APP_BUNDLE/Contents/Resources/Noizee.sdef"
fi

# Copy app icon (.icon bundle for macOS 26+ Liquid Glass, .icns as fallback)
ICON_SOURCE="$ROOT/Sources/Noizee/Resources/noizee.icon"
if [[ -d "$ICON_SOURCE" ]]; then
  echo "🎨 Copying app icon..."
  cp -R "$ICON_SOURCE" "$APP_BUNDLE/Contents/Resources/noizee.icon"
fi
ICNS_PATH="$ROOT/Sources/Noizee/Resources/noizee.icns"
if [[ -f "$ICNS_PATH" ]]; then
  cp "$ICNS_PATH" "$APP_BUNDLE/Contents/Resources/noizee.icns"
fi

# Compile asset catalog if actool is available
XCASSETS_PATH="$ROOT/Sources/Noizee/Resources/Assets.xcassets"
...
${APP_LOCALIZATIONS_PLIST}
    ...
    <string>Copyright © 2026 Alberto Galeazzo. All rights reserved.</string>
    ...
    <array>
        <dict>
            ...
            <string>com.betogzo.noizee</string>
    ...
    <key>SUFeedURL</key>
    <string>https://raw.githubusercontent.com/betogzo/noizee/main/appcast.xml</string>
    ...
    <string>Noizee.sdef</string>

    <!-- Core Audio process tap (Equalizer) - macOS 14.2+ TCC requires these -->
    <key>NSAudioCaptureUsageDescription</key>
    <string>Noizee processes its own music output through a built-in equalizer. This permission only covers Noizee's own playback — no other app's audio is captured.</string>
    <key>NSScreenCaptureUsageDescription</key>
    <string>Noizee taps its own audio output (not the screen) so the built-in equalizer can apply effects to your music. No screen content is recorded.</string>

    <!-- Build Metadata -->
    <key>NoizeeBuildTimestamp</key>
    <string>${BUILD_TIMESTAMP}</string>
    <key>NoizeeGitCommit</key>
...
if [[ "$SIGNING_MODE" == "adhoc" ]]; then
  ...
  if [[ -n "${APP_IDENTITY:-}" ]]; then
    ...
    # `awk` short-circuits on first match and exits 0 even when no line matches,
    # so this stays compatible with `set -euo pipefail` when no cert exists.
    CODESIGN_ID=$(security find-identity -v -p codesigning | awk '/Apple Development/ {print $2; exit}')
  fi
...
if [[ -f "$ROOT/Noizee.entitlements" ]]; then
  codesign "${CODESIGN_ARGS[@]}" --entitlements "$ROOT/Noizee.entitlements" "$APP_BUNDLE"
else
