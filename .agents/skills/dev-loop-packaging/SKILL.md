...
description: Use when you need a fresh packaged `.app` bundle, a fast build-package-relaunch loop, or a packaged runtime repro for Noizee instead of a compile-only verification.
metadata:
  short-description: Package and relaunch Noizee
---
...
- Prefer `swift test --skip NoizeeUITests` for the default non-UI test pass.
...
- `Scripts/build-app.sh` builds the app and assembles `.build/app/Noizee.app`.
- `Scripts/compile_and_run.sh` kills existing Noizee processes, optionally runs tests and linting, packages the app, relaunches it, and checks that it stays running.
