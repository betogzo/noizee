...
This document provides detailed information about Noizee's architecture, services, and design patterns.
...
```
Sources/
  └── Noizee/            → Main app target
      ├── NoizeeApp.swift    → App entry point
...
```swift
// Sources/Noizee/Services/Protocols.swift
...
**File**: `Sources/Noizee/Services/WebKit/WebKitManager.swift`
...
**File**: `Sources/Noizee/Services/Auth/AuthService.swift`
...
**File**: `Sources/Noizee/Services/API/YTMusicClient.swift`
...
**Directory**: `Sources/Noizee/Services/API/Parsers/`
...
**File**: `Sources/Noizee/Services/Player/PlayerService.swift`
...
**File**: `Sources/Noizee/Views/MiniPlayerWebView.swift`
...
**File**: `Sources/Noizee/Services/Player/NowPlayingManager.swift`
...
**File**: `Sources/Noizee/Services/HapticService.swift`
...
**File**: `Sources/Noizee/Services/FavoritesManager.swift`
...
**Persistence**:
- **Location**: `~/Library/Application Support/Noizee/favorites.json`
...
**Related Files**:
- `Sources/Noizee/Models/FavoriteItem.swift` — Data model with `ItemType` enum
- `Sources/Noizee/Views/SharedViews/FavoritesSection.swift` — Horizontal scrolling UI
- `Sources/Noizee/Views/SharedViews/FavoritesContextMenu.swift` — Shared context menu items

### NotificationService

**File**: `Sources/Noizee/Services/Notification/NotificationService.swift`
...
**File**: `Sources/Noizee/Services/NetworkMonitor.swift`
...
**File**: `Sources/Noizee/Services/SettingsManager.swift`
...
**File**: `Sources/Noizee/Services/Audio/EqualizerService.swift`
...
> ⚠️ **Naming note**: `Sources/Noizee/Views/SharedViews/EqualizerView.swift` is unrelated — it's a small now-playing animation indicator (three bouncing bars). The settings UI is `EqualizerSettingsView`.

### SyncedLyricsService

**File**: `Sources/Noizee/Services/Lyrics/SyncedLyricsService.swift`
...
**Related Files**:
- `Sources/Noizee/Services/Lyrics/LyricsProvider.swift` — Provider protocol and search model
- `Sources/Noizee/Services/Lyrics/Providers/LRCLibProvider.swift` — External synced lyrics provider
- `Sources/Noizee/Services/API/Parsers/LRCParser.swift` — LRC to `SyncedLyrics` parser

**Integration**: Created once in `NoizeeApp` and injected through the SwiftUI environment for lyrics views.

See [ADR-0012: Synced Lyrics Provider Architecture](adr/0012-synced-lyrics-architecture.md) for design details.

### ShareService

**File**: `Sources/Noizee/Services/ShareService.swift`
...
**File**: `Sources/Noizee/Services/SongLikeStatusManager.swift`
...
**File**: `Sources/Noizee/Services/URLHandler.swift`

Parses YouTube Music and custom `noizee://` URLs:
...
| `noizee://play?v=xxx` | `.song(videoId:)` |
| `noizee://playlist?list=xxx` | `.playlist(id:)` |
| `noizee://album?id=xxx` | `.album(id:)` |
| `noizee://artist?id=xxx` | `.artist(id:)` |

**Usage**: Called from `NoizeeApp.onOpenURL` to handle deep links.

### ScriptCommands (AppleScript)

**File**: `Sources/Noizee/Services/Scripting/ScriptCommands.swift`
...
**Related Files**:
- `Sources/Noizee/Noizee.sdef` — AppleScript dictionary definition
- `Sources/Noizee/Info.plist` — `NSAppleScriptEnabled` and `OSAScriptingDefinition` keys

**Usage**:
```applescript
tell application "Noizee"
    play
    set volume 50
...
**File**: `Sources/Noizee/Services/UpdaterService.swift`
...
**File**: `Sources/Noizee/AppDelegate.swift`
...
**File**: `Sources/Noizee/Models/YTMusicError.swift`
...
The `Sources/Noizee/Utilities/` folder contains shared helper components.

### ImageCache

**File**: `Sources/Noizee/Utilities/ImageCache.swift`
...
**File**: `Sources/Noizee/Utilities/ColorExtractor.swift`
...
**File**: `Sources/Noizee/Utilities/AnimationConfiguration.swift`
...
**File**: `Sources/Noizee/Utilities/AccessibilityIdentifiers.swift`
...
**File**: `Sources/Noizee/Utilities/IntelligenceModifier.swift`
...
**File**: `Sources/Noizee/Utilities/RetryPolicy.swift`
...
**File**: `Sources/Noizee/Services/API/APICache.swift`
...
Noizee integrates Apple's on-device Foundation Models framework for AI-powered features. See [ADR-0005: Foundation Models Architecture](adr/0005-foundation-models-architecture.md) for detailed design decisions.
...
|-----------|----------|---------|
| `FoundationModelsService` | `Sources/Noizee/Services/AI/` | Singleton managing AI availability and sessions |
| `@Generable` Models | `Sources/Noizee/Models/AI/` | Type-safe structured outputs (`MusicIntent`, `LyricsSummary`, etc.) |
| Tools | `Sources/Noizee/Services/AI/Tools/` | Ground AI responses in real catalog data |
| `AIErrorHandler` | `Sources/Noizee/Services/AI/` | User-friendly error messages |
| `RequiresIntelligenceModifier` | `Sources/Noizee/Utilities/` | Hide AI features when unavailable |

### AI Tools

Tools ground AI responses in real data from the YouTube Music catalog, preventing hallucination.

**MusicSearchTool** (`Sources/Noizee/Services/AI/Tools/MusicSearchTool.swift`)
...
**QueueTool** (`Sources/Noizee/Services/AI/Tools/QueueTool.swift`)
...
| Model | File | Purpose |
|-------|------|---------|
| `MusicIntent` | `Sources/Noizee/Models/AI/MusicIntent.swift` | Parsed user commands (play, pause, search, etc.) |
| `LyricsSummary` | `Sources/Noizee/Models/AI/LyricsSummary.swift` | Lyrics explanation with themes and mood |
| `PlaylistChanges` | `Sources/Noizee/Models/AI/PlaylistChanges.swift` | Queue refinement operations |
...
**File**: `Sources/Noizee/Views/QueueView.swift`
...
**File**: `Sources/Noizee/Views/LyricsView.swift`
...
**File**: `Sources/Noizee/Views/CommandBarView.swift`
...
**File**: `Sources/Noizee/Views/ToastView.swift`
...
**Directory**: `Sources/Noizee/Views/SharedViews/`
...
**File**: `Sources/Noizee/Views/PlayerBar.swift`
...
**File**: `Sources/Noizee/Views/Sidebar.swift`
