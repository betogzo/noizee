...
> **Complete documentation of YouTube Music API endpoints for Noizee development.**
>
...
These endpoints are functional but not yet implemented in Noizee.
...
All parsers are located in `Sources/Noizee/Services/API/Parsers/`. Each parser is responsible for extracting structured data from raw API JSON responses.
...
Noizee uses a unified `YTMusicError` enum for all API-related errors. This enables consistent error handling, user-friendly messages, and retry logic.
...
For authenticated endpoints (🔐), sign in to the Noizee app first:
...
Debug builds export auth cookies for the API explorer to `~/Library/Application Support/Noizee/cookies.dat`.
...
- [Sources/Noizee/Models/MusicVideoType.swift](../Sources/Noizee/Models/MusicVideoType.swift) - Enum definition
- [Sources/Noizee/Models/Song.swift](../Sources/Noizee/Models/Song.swift) - `musicVideoType` property
- [Sources/Noizee/Services/API/Parsers/SongMetadataParser.swift](../Sources/Noizee/Services/API/Parsers/SongMetadataParser.swift) - Parsing logic
...
> ⚠️ **Not Implemented**: Due to DRM requirements, Noizee uses WebView for playback. Direct URL streaming would bypass DRM protection. Quality selection would need to be implemented via WebView JavaScript.
