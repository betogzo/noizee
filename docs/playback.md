...
Noizee's queue or whether YouTube autoplay needs to be corrected.
...
queue after Noizee has already loaded the next track.
...
func loadVideo(videoId: String) {
    ...
    webView.evaluateJavaScript("document.querySelector('video')?.pause()") { _, _ in
        webView.evaluateJavaScript("window.__noizeeTargetVolume = currentVolume")
        self.webView?.load(URLRequest(url: watchURL))
    }
}
```

When the WebView reports a new `videoId`, Noizee treats that as authoritative
...
Noizee treats its own queue as the source of truth whenever one exists:
...
- If YouTube advances to an unexpected track near the end of a song, Noizee
  replays the expected queue track instead of inheriting YouTube autoplay
- At the end of a non-repeating queue, Noizee marks playback ended rather than
