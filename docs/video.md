...
|-----------|------|---------|
| `VideoWindowController` | `Sources/Noizee/VideoWindowController.swift` | Manages the floating video window lifecycle |
| `VideoPlayerWindow` | `Sources/Noizee/Views/VideoPlayerWindow.swift` | SwiftUI view for video display |
| `VideoContainerView` | `Sources/Noizee/Views/VideoPlayerWindow.swift` | NSView container that hosts the WebView |
| `SingletonPlayerWebView+VideoMode` | `Sources/Noizee/Views/SingletonPlayerWebView+VideoMode.swift` | CSS injection for video extraction |
...
3. **Create Container**: Creates `#noizee-video-container` with `position: fixed`
...
1. `window.__noizeeTargetVolume` stores the app's target volume
...
**Solution**: Inject a blackout overlay (`#noizee-blackout`) immediately when entering video mode. This black `<div>` covers the entire viewport until video extraction is complete.
...
3. Observer script applies `__noizeeTargetVolume` when video element is first detected
...
**Files**:
- [Sources/Noizee/Models/MusicVideoType.swift](../Sources/Noizee/Models/MusicVideoType.swift) — Enum definition with `hasVideoContent` property
- [Sources/Noizee/Services/API/Parsers/SongMetadataParser.swift](../Sources/Noizee/Services/API/Parsers/SongMetadataParser.swift) — `parseMusicVideoType(from:)` method
- [Sources/Noizee/Services/Player/PlayerService+Library.swift](../Sources/Noizee/Services/Player/PlayerService+Library.swift) — Calls `updateVideoAvailability(hasVideo:)` after parsing
...
**File**: [Sources/Noizee/Views/SingletonPlayerWebView+ObserverScript.swift](../Sources/Noizee/Views/SingletonPlayerWebView+ObserverScript.swift)
