...
Noizee has no localization infrastructure. All ~300 user-facing strings are hardcoded English literals in SwiftUI views, models, and services. Arabic is the first target language, with potential for additional languages later.

Requirements:
1. **Minimal disruption** — Adding localization should not require architectural changes
2. **SwiftPM-first compatibility** — Noizee builds primarily via SwiftPM, with checked-in Xcode projects for app and UI-test workflows
...
The repo is SwiftPM-first, but it also includes `Noizee.xcodeproj` and `NoizeeUITests.xcodeproj` for app packaging, runtime debugging, and UI-test workflows.
...
The catalog lives at `Sources/Noizee/Resources/Localizable.xcstrings` and is processed by the existing `.process("Resources")` rule in `Package.swift`.
