...
**Issue:** [GitHub Issue #42](https://github.com/betogzo/noizee/issues/42) - AirPlay does not work
...
1. **`Sources/Noizee/Views/SingletonPlayerWebView+PlaybackControls.swift`** - Add `showAirPlayPicker()` method
2. **`Sources/Noizee/Views/PlayerBar.swift`** - Replace `AVRoutePickerView` with custom button
3. **`Sources/Noizee/Services/Player/PlayerService.swift`** - Add `showAirPlayPicker()` method to call through to SingletonPlayerWebView

### Step 1: Add JavaScript injection method

**File:** `Sources/Noizee/Views/SingletonPlayerWebView+PlaybackControls.swift`
...
**File:** Wherever `AccessibilityID.PlayerBar` is defined (likely `Sources/Noizee/Constants/AccessibilityID.swift` or similar)
