...
This document covers testing strategies, commands, and best practices for Noizee.

> 🔁 **Default local loop lives in `AGENTS.md`** — Stay CLI-first for everyday verification: `swift build`, `swift test --skip NoizeeUITests`, then `swiftlint --strict && swiftformat .`. Use Xcode/`xcodebuild` only for UI/runtime debugging, scheme-specific investigation, or CI parity.
...
New code in `Sources/Noizee/` (Services, Models, ViewModels, Utilities) must include unit tests.

### Creating a Test File

1. Create test file in `Tests/NoizeeTests/` matching the source file name
...
swift test --skip NoizeeUITests --filter Parser
```

If you need Xcode's logs or scheme-specific filtering, escalate to:

```bash
xcodebuild test -scheme Noizee -only-testing:NoizeeTests -skip-testing:NoizeeUITests
```
...
```swift
// Tests/NoizeeTests/Helpers/MockYTMusicClient.swift
...
```swift
// Tests/NoizeeTests/Helpers/MockURLProtocol.swift
MockURLProtocol.requestHandler = { request in
...
```swift
// Tests/NoizeeTests/Helpers/TestFixtures.swift
...
xcodebuild test -scheme Noizee -destination 'platform=macOS' \
  -only-testing:NoizeeTests -skip-test-tag integration

# Scheduled job: Run integration tests (may need re-runs)
xcodebuild test -scheme Noizee -destination 'platform=macOS' \
  -only-testing:NoizeeTests/MusicIntentIntegrationTests
```
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build
        run: xcodebuild -scheme Noizee -destination 'platform=macOS' build

      - name: Test
        run: xcodebuild -scheme Noizee -destination 'platform=macOS' test
