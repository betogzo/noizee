...
struct PlayerServiceWebQueueSyncTests {
    ...
    func nearEndAutoplayWithRepeatOneDoesNotAdvanceQueue() async {
        ...
        self.playerService.cycleRepeatMode()
        #expect(self.playerService.repeatMode == .one)
        self.playerService.isNoizeeInitiatedPlayback = false
    ...
    @Test("Shuffle next never keeps the same queue index when multiple tracks exist")
    func shuffleNextAlwaysMovesToDifferentRow() async {
        let songs = [
            Song(id: "1", title: "Song 1", artists: [], album: nil, duration: 180, thumbnailURL: nil, videoId: "v1"),
            Song(id: "2", title: "Song 2", artists: [], album: nil, duration: 200, thumbnailURL: nil, videoId: "v2"),
            Song(id: "3", title: "Song 3", artists: [], album: nil, duration: 220, thumbnailURL: nil, videoId: "v3"),
        ...
        for _ in 0 ..< 24 {
            let indexBefore = self.playerService.currentIndex
            await self.playerService.next()
            #expect(self.playerService.currentIndex != indexBefore)
            #expect(self.playerService.queue.indices.contains(self.playerService.currentIndex))
        }
    }

    @Test("UpdateTrackMetadata corrects YouTube autoplay with Noizee-initiated playback")
    func updateTrackMetadataCorrectsYouTubeAutoplay() async {
        ...
        // Simulate calling next which sets isNoizeeInitiatedPlayback
        await self.playerService.next()

        // Get the song that Noizee intended to play
