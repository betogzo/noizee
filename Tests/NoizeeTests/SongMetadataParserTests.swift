...
struct SongMetadataParserTests {
    ...
    @Test("parseArtists skips album browse runs and release year text")
    func parseArtistsSkipsAlbumAndYearRuns() {
        let renderer: [String: Any] = [
            "longBylineText": [
                "runs": [
                    [
                    ...
                    [
                        "text": "Midnights",
                        "navigationEndpoint": [
                            "browseEndpoint": ["browseId": "MPREb_albumtest"],
                        ],
                    ],
                    ["text": " • "],
                    ["text": "2022"],
        ...
        #expect(artists.count == 1)
        #expect(artists[0].name == "Taylor Swift")
        #expect(artists[0].id == "UC123")
    }

    @Test("parseAlbum extracts album from long byline")
    func parseAlbumExtractsAlbumFromByline() {
        let renderer: [String: Any] = [
            "longBylineText": [
                "runs": [
                    [
        ...
        let album = SongMetadataParser.parseAlbum(from: renderer)

        #expect(album?.id == "MPREb_albumtest")
        #expect(album?.title == "Midnights")
    }
