...
struct ParsingHelpersTests {
    ...
    // MARK: - Videos shelf titles

    @Test(
        "Videos category shelf title detection respects locale variants",
        arguments: ["Videos", "Vídeos", "Music videos", "  videos  ", "Vidéos"]
    )
    func isVideosCategoryShelfTitlePositive(raw: String) {
        #expect(ParsingHelpers.isVideosCategoryShelfTitle(raw) == true)
    }

    @Test(
        "Videos category shelf title detection rejects non-video shelves",
        arguments: ["Songs", "New releases", "Quick picks"]
    )
    func isVideosCategoryShelfTitleNegative(raw: String) {
        #expect(ParsingHelpers.isVideosCategoryShelfTitle(raw) == false)
    }

    // MARK: - Album extraction

    @Test("extractAlbumFromSubtitleRuns returns album with MPRE browse id")
    func extractAlbumFromSubtitleRunsFindsAlbum() {
        let data: [String: Any] = [
            "subtitle": ["runs": [
                ["text": "A", "navigationEndpoint": ["browseEndpoint": ["browseId": "UCartist"]]],
                ["text": " • "],
                ["text": "The Album", "navigationEndpoint": ["browseEndpoint": ["browseId": "MPREb_subtitle"]]],
            ]],
        ]
        let album = ParsingHelpers.extractAlbumFromSubtitleRuns(data)
        #expect(album?.id == "MPREb_subtitle")
        #expect(album?.title == "The Album")
    }

    @Test("browseShelfTitle reads musicShelfBasicHeaderRenderer")
    func browseShelfTitleFromShelfHeader() {
        ...
        #expect(ParsingHelpers.browseShelfTitle(forRenderer: renderer) == "Featured")
    }
    ...
    func extractArtistsFiltersSeparators() {
        ...
        #expect(artists[0].name == "Neil Young")
    }

    @Test("Extract artists skips music content type labels in subtitle")
    func extractArtistsSkipsContentTypeLabels() {
        let data: [String: Any] = [
            "subtitle": [
                "runs": [
                    ["text": "Song"],
                    ["text": ", "],
                    ["text": "Dream Theater", "navigationEndpoint": ["browseEndpoint": ["browseId": "UCdream"]]],
                ],
            ],
        ]
        ...
        #expect(artists.count == 1)
        #expect(artists[0].name == "Dream Theater")
        #expect(artists[0].id == "UCdream")
    }

    @Test("isMusicContentTypeLabel is case-insensitive and trims punctuation")
    func isMusicContentTypeLabelNormalization() {
        #expect(ParsingHelpers.isMusicContentTypeLabel("Song"))
        #expect(ParsingHelpers.isMusicContentTypeLabel("song"))
        #expect(ParsingHelpers.isMusicContentTypeLabel("EP,"))
        #expect(!ParsingHelpers.isMusicContentTypeLabel("Dream Theater"))
    }
    ...
    // MARK: - YouTube Web Player Bar Byline

    @Test("primaryArtistLine strips album and year from YouTube byline")
    func primaryArtistLineStripsAlbumAndYear() {
        let byline = "Johnny Cash • American IV • 2002"
        #expect(ParsingHelpers.primaryArtistLine(fromYouTubeMusicPlayerBarByline: byline) == "Johnny Cash")
    }

    @Test("primaryArtistLine returns full string when no album segment")
    func primaryArtistLinePassthroughSingleArtist() {
        #expect(ParsingHelpers.primaryArtistLine(fromYouTubeMusicPlayerBarByline: "Kraftwerk") == "Kraftwerk")
    }

    @Test("primaryArtistLine handles bullet without surrounding spaces")
    func primaryArtistLineTightBullet() {
        #expect(ParsingHelpers.primaryArtistLine(fromYouTubeMusicPlayerBarByline: "Artist•Album Name") == "Artist")
    }
}
