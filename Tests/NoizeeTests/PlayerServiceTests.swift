...
struct PlayerServiceTests {
    ...
    @Test("Update track metadata normalizes YouTube player bar byline to primary artist only")
    func updateTrackMetadataNormalizesYouTubeByline() {
        self.playerService.updateTrackMetadata(
            title: "Track",
            artist: "The Velvet Underground • Loaded • 1970",
            thumbnailUrl: "",
            videoId: "v-byline"
        )

        #expect(self.playerService.currentTrack?.artistsDisplay == "The Velvet Underground")
    }

    @Test("Update track metadata keeps queue artists when web byline is briefly empty")
    func updateTrackMetadataKeepsQueueArtistsWhenBylineEmpty() {
        let channelArtist = Artist(
            id: "UCbriefemptybyline",
            name: "Black Sabbath",
            thumbnailURL: nil,
            subtitle: nil,
            profileKind: .artist
        ...
        let song = Song(
            ...
            duration: nil,
            thumbnailURL: nil,
            videoId: "v-brief-empty"
        )
        self.playerService.setQueue([song])
        ...
        #expect(self.playerService.currentTrack?.artistsDisplay == "Black Sabbath")
    ...
    @Test("Keeping queue row visible does not strip navigable album enriched before Web metadata arrives")
    func updateTrackMetadataKeepsEnrichedAlbumWhenPreservingQueueRow() {
        let album = Album(
            id: "MPREb_queuekeepalbum",
        ...
        let queueSong = Song(
            ...
            videoId: "v-queue-keep"
        )
        self.playerService.setQueue([queueSong])
        self.playerService.currentIndex = 0
        self.playerService.isNoizeeInitiatedPlayback = true
        ...
        #expect(self.playerService.currentTrack?.videoId == "v-queue-keep")
        #expect(self.playerService.currentTrack?.album?.id == album.id)
        #expect(self.playerService.currentTrack?.album?.hasNavigableId == true)
        #expect(self.playerService.isNoizeeInitiatedPlayback == false)
    }
