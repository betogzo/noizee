...
struct HomeResponseParserTests {
    ...
    @Test("Parse twoRow song extracts navigable album from subtitle runs")
    func parseTwoRowSongExtractsAlbumFromSubtitle() throws {
        let songData: [String: Any] = [
            "musicTwoRowItemRenderer": [
                "title": ["runs": [["text": "Carousel Song"]]],
                "navigationEndpoint": [
                    "watchEndpoint": ["videoId": "vid-carousel-album"],
                ],
                "subtitle": ["runs": [
                    ["text": "Some Artist", "navigationEndpoint": ["browseEndpoint": ["browseId": "UCcarouselartist"]]],
                    ["text": " • "],
                    ["text": "Carousel Album", "navigationEndpoint": ["browseEndpoint": ["browseId": "MPREb_carousel"]]],
                ]],
            ],
        ]
        let sectionData: [String: Any] = [
        ...
        let section = try #require(HomeResponseParser.parseHomeSection(sectionData))
        if case let .song(song) = section.items.first {
            #expect(song.videoId == "vid-carousel-album")
            #expect(song.album?.id == "MPREb_carousel")
            #expect(song.album?.title == "Carousel Album")
            #expect(song.album?.hasNavigableId == true)
        } else {
            Issue.record("Expected song item")
        }
    }
    ...
    @Test("Excluding Videos shelf drops carousel section from home parsing")
    func excludeVideosShelfFromCarousel() {
        let placeholderAlbum: [String: Any] = [
            "musicTwoRowItemRenderer": [
                "title": ["runs": [["text": "VMA Clip"]]],
                "navigationEndpoint": [
                    "browseEndpoint": [
                        "browseId": "MPREVMA",
                        "browseEndpointContextSupportedConfigs": [
        ...
        let sectionData: [String: Any] = [
            "musicCarouselShelfRenderer": [
                ...
                "contents": [placeholderAlbum],
            ],
        ]

        let included = HomeResponseParser.parseHomeSection(sectionData, excludeVideosCategoryShelves: false)
        let excluded = HomeResponseParser.parseHomeSection(sectionData, excludeVideosCategoryShelves: true)
