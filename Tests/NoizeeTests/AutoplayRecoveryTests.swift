...
struct AutoplayRecoveryJSTests {
    private func makeContext() -> JSContext {
        ...
        // loading the recovery function (which references `window.__noizee…`).
    ...
    func clicksButtonWhenFlagPendingAndPaused() {
        ...
        ctx.evaluateScript(
            """
            window.__noizeeAutoplayPending = true;
            var clicked = false;
            var played = false;
            var video = { paused: true, play: function() { played = true; } };
            var btn = { click: function() { clicked = true; } };
            globalThis.result = __noizeeAttemptAutoplayRecovery(video, btn);
            """
        )
        ...
        #expect(ctx.evaluateScript("window.__noizeeAutoplayPending").toBool() == false)
    }
    ...
    func fallsBackToVideoPlay() {
        ...
        ctx.evaluateScript(
            ...
            var played = false;
            var video = { paused: true, play: function() { played = true; } };
            globalThis.result = __noizeeAttemptAutoplayRecovery(video, null);
            """
        )
    ...
    func observerScriptEmbedsRecoveryFunction() {
        #expect(SingletonPlayerWebView.observerScript.contains("__noizeeAttemptAutoplayRecovery"))
    }

    @Test("Observer script clears autoplay intent on successful playback")
    func observerScriptClearsAutoplayIntentOnPlayback() {
        #expect(SingletonPlayerWebView.observerScript.contains("window.__noizeeAutoplayPending = false;"))
    }
...
struct AutoplayIntentScriptTests {
    ...
    func setsPendingTrue() {
        ...
        #expect(script == "window.__noizeeAutoplayPending = true;")
    }
    ...
    func clearsPendingForRestoredSession() {
        ...
        #expect(script == "window.__noizeeAutoplayPending = false;")
    }
    ...
    func pageBootstrapSeedsIntentAndTargetVolume() {
        ...
        #expect(script.contains("window.__noizeeAutoplayPending = true;"))
        #expect(script.contains("window.__noizeeTargetVolume = 0.42;"))
    }
    ...
    func pageBootstrapClampsInvalidTargetVolume() {
        ...
        #expect(script.contains("window.__noizeeTargetVolume = 1.0;"))
    }
}
