...
enum TestAccessibilityID {
    ...
    enum MainWindow {
        ...
        static let toolbarSearchField = "mainWindow.toolbarSearch"
...
// MARK: - NoizeeUITestCase

/// Base class for Noizee UI tests.
/// Provides common setup, launch configuration, and helper methods.
@MainActor
class NoizeeUITestCase: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        // Create new app instance pointing to installed Noizee.app
        let appURL = URL(fileURLWithPath: "/Applications/Noizee.app")
        if FileManager.default.fileExists(atPath: appURL.path) {
            self.app = XCUIApplication(url: appURL)
        } else {
            self.app = XCUIApplication(bundleIdentifier: "com.betogzo.Noizee")
        }
    ...
    /// Navigates to Search via keyboard shortcut (sidebar item was removed).
    func navigateToSearch() {
        self.app.typeKey("f", modifierFlags: .command)
        let searchTitle = self.app.staticTexts["Search"]
        _ = self.waitForElement(searchTitle, timeout: 10)
    }
