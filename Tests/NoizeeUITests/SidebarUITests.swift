...
@MainActor
final class SidebarUITests: NoizeeUITestCase {
    // MARK: - Navigation Items Visible

    func testSidebarShowsAllNavigationItems() {
        launchDefault()

        // Verify sidebar items are present via accessibility identifiers
        ...
        XCTAssertTrue(homeItem.waitForExistence(timeout: 10), "Home item should exist")
        ...
        navigateToHome()
        let toolbarSearch = app.textFields[TestAccessibilityID.MainWindow.toolbarSearchField]
        XCTAssertTrue(toolbarSearch.waitForExistence(timeout: 5), "Toolbar search field should exist on Home")

        navigateToExplore()
        let toolbarSearchOnExplore = app.textFields[TestAccessibilityID.MainWindow.toolbarSearchField]
        XCTAssertTrue(toolbarSearchOnExplore.waitForExistence(timeout: 5), "Toolbar search field should exist on Explore")
    }
